...
async function fetchComponent() {
  loading.value = true
  try {
    const data = await api<{ success: boolean; component: any }>(`/api/components/${id}`)
    component.value = data.component
  } catch (err) {
    console.error('Fetch component detail error:', err)
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="page-container fade-in">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          <div class="breadcrumb">
            <NuxtLink to="/components">Linh kiện</NuxtLink>
            <span class="separator">/</span>
            <span>Chi tiết</span>
          </div>
          <h1 class="comp-title" v-if="component">{{ component.name }}</h1>
    ...
    <div v-if="loading" class="loading-state">
      ...
      <p>Đang tải thông tin linh kiện...</p>
    </div>

    <div v-else-if="component" class="detail-grid">
      <!-- Left Column -->
      <div class="main-column">
        <div class="info-card stats-card">
          <div class="stat-item">
            <span class="stat-label">Trạng thái</span>
            <span :class="['status-badge', `status-${component.status.toLowerCase()}`]">
              {{ getStatusLabel(component.status) }}
            </span>
          </div>
          <div class="stat-item border-l border-r border-white/5">
            <span class="stat-label">Thiết bị hiện tại</span>
            <span class="stat-value">
              <NuxtLink v-if="component.device" :to="`/devices/${component.device.id}`" class="text-accent underline">
          ...
          <div class="stat-item">
            ...
            <span class="stat-value">{{ component.location?.name || 'Chưa xác định' }}</span>
          </div>
        </div>
        ...
        <div class="info-card">
          ...
          <div class="details-grid">
            ...
            <div class="detail-item">
              ...
              <span class="category-tag">{{ component.category?.name }}</span>
            </div>
            <div class="detail-item">
        ...
        <div class="info-card">
          ...
          <p class="text-sm italic text-secondary">{{ component.notes || 'Không có ghi chú.' }}</p>
        </div>
      </div>
      ...
      <div class="side-column">
        <div class="info-card">
          <Tabs v-model="activeTab" :tabs="tabs">
            <div v-if="activeTab === 'installations'" class="history-content">
              <div v-if="component.installations.length === 0" class="empty-history">
                Chưa có lịch sử lắp đặt.
              </div>
              <div v-else class="timeline">
                <div v-for="inst in component.installations" :key="inst.id" class="timeline-item">
                  <div class="timeline-dot" :class="{ 'active': !inst.uninstalledAt }"></div>
                  <div class="timeline-body">
                    <div class="flex justify-between items-start">
                      <NuxtLink :to="`/devices/${inst.device.id}`" class="user-name underline">{{ inst.device.name }}</NuxtLink>
                    </div>
                    <div class="timeline-dates">
                      <span>{{ formatDate(inst.installedAt) }}</span>
                      <span v-if="inst.uninstalledAt"> → {{ formatDate(inst.uninstalledAt) }}</span>
                      <span v-else class="current-tag ml-2">Đang lắp</span>
                    </div>
                  </div>
                </div>
              </div>
...
.comp-title { font-size: var(--font-size-3xl); font-weight: 800; }
...
.user-name { font-size: var(--font-size-sm); font-weight: 700; color: var(--color-text-primary); text-decoration: none; }
.timeline-dates { font-size: var(--font-size-xs); color: var(--color-text-muted); }
.current-tag { background: rgba(99, 102, 241, 0.1); color: var(--color-accent); font-size: 10px; font-weight: 700; padding: 2px 6px; border-radius: 4px; text-transform: uppercase; }

.status-badge { display: inline-flex; padding: 4px 10px; border-radius: var(--radius-full); font-size: var(--font-size-xs); font-weight: 600; }
